<div class='btn-group'>
    @if( App\Models\Admin\Question::checkQuestionsTypeHaveAnswers($question->question_type))
        <a type="button" href="{{ route('teacher.answers.index',['question_id'=>$question->id]) }}"
           class="btn btn-primary mx-2 button-icon mb-1 btn-sm"><i
                class="fa fa-question-circle me-2"></i> {{ trans('models/answers.plural') }}
        </a>
    @endif
    <a type="button" href="{{ route('teacher.questions.show', [$question->id,'lesson_id'=>$question->lesson_id]) }}" class="btn btn-info mx-2 button-icon mb-1 btn-sm"><i
            class="fe fe-eye me-2"></i> {{__('lang.show')}}</a>
    <a type="button" href="{{ route('teacher.questions.edit', [$question->id,'lesson_id'=>$question->lesson_id]) }}"
    ...
    <form id="Row{{$question->id}}"
          action="{{ route('teacher.questions.destroy', [$question->id,'lesson_id'=>$question->lesson_id]) }}"
